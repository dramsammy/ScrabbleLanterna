...
public class Board{
  ...
  public boolean modifyBoard(int xcor, int ycor,char newval){//will start from zero
    if(board[ycor][xcor].getPieceChar()!=' ' && board[ycor][xcor].getPieceChar()!=newval){
      return false;
    }
    else{
      board[ycor][xcor].setLetter(newval);
      return true;
    }
  }
...
public boolean makePlay(Pieces[] p,int xcor,int ycor, int direction, Player l, boolean firstturn){
  if(isValidPlay(p,xcor,ycor,direction) && ((!firstturn) || crossesOrigin(p,xcor,ycor,direction))){
    if(direction==1){
...
public boolean isValidPlay(Pieces[] p,int xcor,int ycor, int direction){
  ...
  for (int a = 0; a < p.length; a++){
    ...
    if (getPieces(x+1,y).getPieceChar()!=' ' && direction == -1){
      ...
      temp = new Pieces[count+p.length];
      f = 0;
      while (getPieces(x + f,y).getPieceChar()!= ' '){
